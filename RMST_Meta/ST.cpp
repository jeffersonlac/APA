...
int ST::proximoNo(Vertice &v1, double alpha)
{
	long int menor = INT32_MAX, maior = -1;
	int ind_menor, ind_maior;
	vector<Vertice*> aux;
	...
	srand(time(NULL));
	double valor = ((maior - menor) * alpha) + menor;
	int aleatorio = ind_menor + rand()%(int)valor;

	// Vertice v2(-1, ((*this->vertices[ind_maior]).get_x() - (*this->vertices[ind_menor]).get_x())*alpha, ((*this->vertices[ind_maior]).get_y() - (*this->vertices[ind_menor]).get_y())*alpha, 0, nullptr);
	...
	// 	if((*this->vertices[i]).get_ac() == false)
	// 	{
	// 		long int dist = this->distancia(v2, (*this->vertices[i]));
	// 		if(dist < menor)
	// 		{
	// 			menor = dist;
	// 			ind = i;
	...
	this->nos_nvisitados--;
	long int dist = this->distancia(v1, (*aux[aleatorio]));
	v1.set_chave(dist);
	return (*aux[aleatorio]).get_id();
};
